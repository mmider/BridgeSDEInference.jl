...
s3, β = textslider(-3.0f0:0.001f0:3.0f0, "s", start = 0.8)
...
function drift(x::Point2f0, P::FitzhughNagumoDiff)
    Point2f0((x[1] - x[2] - x[1]^3 + P.s)/P.ϵ, P.γ*x[1] - x[2])
end
...
pline = lift(γ) do γ
  [x for x in Point2f0.(r, γ*r) if x in limits]
end
pline2 = lift(β) do s
    [x for x in Point2f0.(r, r - r.^3 .+ s) if x in limits]
end

# null klines
lines!(particles_canvas, pline2, color = RGBA{Float32}(0.5, 0.7, 1.0, 0.5))
